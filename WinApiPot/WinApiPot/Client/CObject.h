...
struct tAttackInfo
{
	float m_fAttackDamage; //데미지
	float m_fAttRcnt;//맞으면 날라갈 양 X
	float m_fAttUpperRcnt; //맞으면 띄어질 양 Y
	float m_fAttUpperAcc; //추가 가속도
	float m_fAttRigidityTime; //경직 시간
	Vec2 m_fAttackRange; //내 기본 공격 범위 (트리거)
	Vec2 m_fSkillRange; //내 스킬 범위 (트리거)
	ATTACK_TYPE m_eAttType; //공격 타입
};
